...
@section('content')
    <div class="kt-card mx-7.5">
        ...
        <div class="kt-card-content">
            <form id="edit-footer-form" action="{{ route('footers.update', $data['footer']->id) }}" method="POST" class="space-y-5">
                @method('PATCH')
                ...
                <div>
                    ...
                    <input type="text" name="name" class="kt-input w-full"
                        value="{{ old('name', $data['footer']->name) }}" placeholder="Masukkan nama" required/>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" name="url" class="kt-input w-full"
                        value="{{ old('url', $data['footer']->url) }}" placeholder="Masukkan url" required />
                    @error('url')
                        <p class="text-sm text-destructive mt-1">{{ $message }}</p>
                    @enderror
                </div>
...
@push('scripts')
    <script>
        document.getElementById('edit-footer-form').addEventListener('submit', function(e) {
            e.preventDefault();

            Swal.fire({
                title: 'Konfirmasi',
                text: 'Apakah Anda yakin ingin mengedit footer ini?',
                icon: 'question',
