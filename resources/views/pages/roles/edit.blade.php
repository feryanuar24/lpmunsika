...
@section('content')
    <div class="kt-card mx-7.5">
        ...
        <div class="kt-card-content">
            <form id="edit-role-form" action="{{ route('roles.update', $data['role']->id) }}" method="POST" class="space-y-5">
                @method('PATCH')
                ...
                <div>
                    ...
                    <input type="text" name="name" class="kt-input w-full" value="{{ old('name', $data['role']->name) }}"
                ...
                <div>
                    ...
                    <input type="text" name="display_name" class="kt-input w-full"
                        value="{{ old('display_name', $data['role']->display_name) }}" placeholder="Masukkan nama" required/>
                    @error('display_name')
                        <p class="text-sm text-destructive mt-1">{{ $message }}</p>
                    @enderror
                </div>
...
@push('scripts')
    <script>
        document.getElementById('edit-role-form').addEventListener('submit', function(e) {
            ...
            Swal.fire({
                title: 'Konfirmasi',
                text: 'Apakah Anda yakin ingin mengedit role ini?',
                icon: 'question',
