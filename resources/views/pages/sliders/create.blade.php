...
@section('content')
    <div class="kt-card mx-7.5">
        ...
        <div class="kt-card-content">
            <form id="create-slider-form" action="{{ route('sliders.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <span class="text-destructive">*</span>
                    <input type="file" name="banner" id="banner" class="kt-input w-full" required/>
                    @error('banner')
                        <p class="text-sm text-destructive mt-1">{{ $message }}</p>
                    @enderror
                </div>
...
@push('scripts')
    <script>
        document.getElementById('create-slider-form').addEventListener('submit', function(e) {
            e.preventDefault();

            Swal.fire({
                title: 'Konfirmasi',
                text: 'Apakah Anda yakin ingin membuat slider baru?',
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        title: 'Memproses...',
                        text: 'Slider baru sedang dibuat.',
