    ...
    <div class="kt-card">
        <div class="kt-card-header">
            <div class="kt-card-heading">
                <h2 class="kt-card-title">Halaman Login</h2>
            </div>
        </div>

        <div class="kt-card-content">
            <form id="login-form" action="{{ route('login') }}" class="space-y-5" method="POST">
                @csrf
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" class="kt-checkbox" id="remember" name="remember" value="{{ old('remember') }}" />
                    <label class="kt-label" for="remember">Ingat Saya</label>
                </div>
                ...
                <!-- Submit Button -->
                <button type="submit" class="kt-btn kt-btn-primary">Masuk</button>
            </form>
        </div>
        ...
        <div class="kt-card-footer">
            <div class="space-y-5 flex flex-col">
                <a href="{{ route('register') }}" class="kt-link-underlined kt-link">Belum punya akun? Daftar
                    sekarang</a>
                <a href="{{ route('password.request') }}" class="kt-link-underlined kt-link">Lupa kata sandi? Ubah kata
                    sandi</a>
            </div>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        ...
        document.getElementById('login-form').addEventListener('submit', function(e) {
            ...
            Swal.fire({
                    title: 'Konfirmasi',
                    text: 'Lanjutkan masuk ke akun Anda?',
                    icon: 'question',
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        Swal.fire({
                            title: 'Memproses...',
                            text: 'Sedang masuk ke akun Anda',
