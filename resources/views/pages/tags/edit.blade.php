...
@section('content')
    <div class="kt-card mx-7.5">
        ...
        <div class="kt-card-content">
            <form id="edit-tag-form" action="{{ route('tags.update', $data['tag']->id) }}" method="POST" class="space-y-5">
                @method('PATCH')
                ...
                <div>
                    ...
                    <input type="text" name="name" class="kt-input w-full"
                        value="{{ old('name', $data['tag']->name) }}" placeholder="Masukkan nama" required/>
                    @error('name')
                ...
                <div>
                    ...
                    <textarea name="description" class="kt-textarea w-full" rows="4" placeholder="Masukkan deskripsi">{{ old('description', $data['tag']->description) }}</textarea>
                    @error('description')
                        <p class="text-sm text-destructive mt-1">{{ $message }}</p>
                    @enderror
                </div>
...
@push('scripts')
    <script>
        document.getElementById('edit-tag-form').addEventListener('submit', function(e) {
            ...
            Swal.fire({
                title: 'Konfirmasi',
                text: 'Apakah Anda yakin ingin mengedit tag ini?',
                icon: 'question',
