...
@section('content')
    <div class="kt-card mx-7.5">
        ...
        <div class="kt-card-content">
            <form id="create-platform-form" action="{{ route('platforms.store') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="icon" class="kt-input w-full" value="{{ old('icon') }}"
                        placeholder="Contoh: ki-instagram" />
                    @error('icon')
                        <p class="text-sm text-destructive mt-1">{{ $message }}</p>
                    @enderror
                </div>
...
@push('scripts')
    <script>
        document.getElementById('create-platform-form').addEventListener('submit', function(e) {
            e.preventDefault();

            Swal.fire({
                title: 'Konfirmasi',
                text: 'Apakah Anda yakin ingin membuat platform baru?',
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        title: 'Memproses...',
                        text: 'Platform baru sedang dibuat.',
